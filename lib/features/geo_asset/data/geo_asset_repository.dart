...
abstract interface class GeoAssetRepository {
  ...
  TaskEither<GeoAssetFailure, ({GeoAssetEntity geoip, GeoAssetEntity geosite})> getActivePair();
...
class GeoAssetRepositoryImpl with ExceptionHandler, InfraLogger implements GeoAssetRepository {
  ...
  TaskEither<GeoAssetFailure, ({GeoAssetEntity geoip, GeoAssetEntity geosite})> getActivePair() {
    return exceptionHandler(
      () async {
        final geoip = await geoAssetDataSource.getActiveAssetByType(GeoAssetType.geoip);
        final geosite = await geoAssetDataSource.getActiveAssetByType(GeoAssetType.geosite);
  ...
  @override
  Stream<Either<GeoAssetFailure, List<GeoAssetWithFileSize>>> watchAll() {
    final persistedStream = geoAssetDataSource.watchAll().map((event) => event.map((e) => e.toEntity()));
    ...
    return Rx.combineLatest2(
      ...
      (assets, files) => assets.map(
        (e) {
          final path = geoAssetPathResolver.file(e.providerName, e.fileName).path;
  ...
  Future<Iterable<File>> _readGeoFiles() async {
    return _geoFiles = Directory(geoAssetPathResolver.directory.path).listSync().whereType<File>().where((e) => e.extension == '.db');
  }
  ...
  TaskEither<GeoAssetFailure, Unit> update(GeoAssetEntity geoAsset) {
    return exceptionHandler(
      () async {
        ...
        final file = geoAssetPathResolver.file(geoAsset.providerName, geoAsset.name);
        ...
        final assets = (response.data!['assets'] as List).whereType<Map<String, dynamic>>();
        final asset = assets.firstOrNullWhere((e) => e["name"] == geoAsset.name);
        if (asset == null) {
          return left(
            GeoAssetUnexpectedFailure.new(
  ...
  TaskEither<GeoAssetFailure, Unit> addRecommended() {
    return exceptionHandler(
      () async {
        final persistedIds = await geoAssetDataSource.watchAll().first.then((value) => value.map((e) => e.id));
        final missing = recommendedGeoAssets.where((e) => !persistedIds.contains(e.id));
